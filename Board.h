...
namespace Chess
{
  class Board {
	...
	public:
		...
		const Piece* get_piece(const Position& position) const;
		...
		bool check_path(const Position& start, const Position& end, char piece_designator) const;

		// Check diagonal path
		bool check_bishop_path(const Position& start, const Position& end) const;

		// Check horizontal/vertical path
		bool check_rook_path(const Position& start, const Position& end) const;

		// Return the map storing the pieces
		const std::map<Position, Piece*>& get_map() const {return occ;}
